---
name: imarflex-business-focus
description: Imarflex 伊瑪牌 business focus operating manual — writes and updates monthly / quarterly business-focus docs that feed topic-research-workflow Step 1. Use when drafting, editing or reviewing a `yyyy-qx-focus.md` / `yyyy-mm-focus.md`, when the user asks about 業務方向, monthly focus, quarterly focus, Q2 focus, May focus, "what should we push this month", or when starting a topic-research cycle. Triggers on "business focus", "monthly focus", "quarterly focus", "Q2 focus", "Q3 focus", "業務方向", "今季要推", "今個月要推", "research direction", "diff from quarter".
...
Upstream input 嚟自 **Step 0(Market intelligence scan)**,由 sibling skill `imarflex-market-scan` + `imarflex-market-scout` agent 負責 — output 係 5-10 條 macro market bullet,直接 paste 入 focus doc 嘅 `## Market context` section。冇 Step 0 input,Step 1 嘅 category 選擇就會變返「client 想 sell 咩 + 季節」嘅 closed loop。
...
- **`imarflex-market-scan`** (Step 0,upstream) — macro market intelligence playbook。Output(5-10 bullet)直接 feed 入 focus doc `## Market context` section。Agent: `imarflex-market-scout`。
- **`imarflex-signal-gather`** (Step 2,downstream) — tactical topic-level signal gathering。Consume focus doc 嘅 thesis 去 weight 邊啲 signal 重要。Agent: `imarflex-signal-scout`。
