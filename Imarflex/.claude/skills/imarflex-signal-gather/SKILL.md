---
name: imarflex-signal-gather
description: Imarflex 伊瑪牌 signal-gathering playbook for topic-research-workflow Step 2 + Step 3. Use to run a weekly Monday signal sweep across the 8 sources (competitor SERP, retailer/marketplace, product catalog, seasonal calendar, customer service/WhatsApp, Google Search Console, site search/Meilisearch, PostHog) and drop raw topic captures into Imarflex/online-marketing/research-inbox/. Pitch-stage aware — runs the 4 sources available without client data and produces a clean "request-from-client" list for the gated 4. Triggers on "gather signals", "research signals", "signal sweep", "Monday signal", "competitor SERP", "retailer wording", "site search", "GSC", "PostHog", "topic research", "research inbox", "信號收集", "topic ideas Imarflex", "imarflex weekly research".
paths: Imarflex/online-marketing/research-inbox/**
...
3. 每條 signal 寫做一個 raw note,放入 research-inbox/yyyy-mm-dd-source-short.md
...
每個 raw signal 寫做一個 markdown file 入 `research-inbox/`,filename:
...
```
business-focus  ─►  imarflex-signal-gather  ─►  research-inbox  ─►  Step 4 scoring  ─►  seo-keyword-research  ─►  content-briefs  ─►  imarflex-copywriter (產出)
                    (呢個 skill)               (我哋嘅 output)
```
...
- **`imarflex-business-focus` skill** — 如要更新本月 focus,delegate 出去。
