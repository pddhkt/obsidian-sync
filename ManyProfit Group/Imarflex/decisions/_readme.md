...
Structured `type: decision` notes — the open questions and decisions-of-record for the build. This folder is what `_open-questions.base` scans (`file.inFolder("ManyProfit Group/Imarflex/decisions")`), so a note only shows up in that dashboard if it lives here.
