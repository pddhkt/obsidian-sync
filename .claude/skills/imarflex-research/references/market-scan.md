...
| **下游** | 餵 `imarflex-research/business-focus` skill 寫 focus | 餵 Step 4 scoring → seo-keyword-research → content-briefs |

> [!warning]
> 兩個 skill **唔可以撈亂**。如果你而家寫緊嘅嘢係「氣炸鍋蝦多士食譜」嘅 keyword / SERP — 嗰個係 Step 2,用 `imarflex-research/signal-gather`。本 skill 只回答「氣炸鍋呢個 category 喺香港 macro 上面係咪上緊」。
...
7. 出「paste-into-focus-doc」snippet 畀 user,提示佢 hand over 去 `imarflex-research/business-focus` skill
...
## How this feeds `imarflex-research/business-focus`
...
├── 證據程度 banner
├── 季節背景
├── ★ Market context(由 imarflex-research/market-scan 提供)  ← 新加
├── 今季要推邊個 category?
...
> [!info]
> `imarflex-research/business-focus` skill 嘅 quarterly / monthly template 需要喺日後 update 加入 Market context section。本 skill 嘅 `references/scan-output-template.md` 已預備好 paste-in 格式。
...
- ❌ 唔做 keyword-level / topic-level signal — 用 `imarflex-research/signal-gather`
- ❌ 唔寫 business focus — 用 `imarflex-research/business-focus` skill;本 skill 只係餵 input
