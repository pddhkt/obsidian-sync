...
├── brand/                   ← voice + visual + content working notes
│   ├── _deliverables.base
│   ├── guideline/           ← client-ready final assets: logos / color-palettes / moodboard
│   ├── social-samples/{product,lifestyle,carousel}/
│   └── templates/
├── decisions/               ← open questions / stack decisions
├── online-marketing/        ← marketing workspace (see online-marketing/_index.md)
│   ├── blog/                ← sample blog posts (pitch material)
│   ├── funnel/              ← 6 stage notes with per-stage mermaid + feature lists
│   ├── business-focus/      ← yyyy-qx / yyyy-mm focus docs
│   ├── research-inbox/YYYY-MM/   ← signal captures, month subfolders
│   ├── reports/YYYY-MM/     ← reviews & reports, month subfolders
│   ├── sales-content/       ← cycles/YYYY-MM/ + go-live-kits/ (campaign assets live WITH the campaign, e.g. go-live-kits/assets/)
│   └── products/<sku>/      ← product truth: specs, approved source images
├── pitch/                   ← client-facing pitch docs + deck/ (HTML deck)
...
│   └── funnel-demo-spec.md
├── site-migration/
└── backups/                 ← frozen snapshots, do not edit
```
